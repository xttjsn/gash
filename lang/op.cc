...
namespace gashlang {
  ...
  int evala_SUB(Bundle& in0, Bundle& in1, Bundle& out) {
    ...
    if (neg_status < 0) return neg_status;
  ...
  int evala_MUL_raw(Bundle& in0, Bundle& in1, Bundle& out) {
    ...
    for (u32 i = 0; i < len; i++) {
      ...
      if (prev_res.size() > 0) {
        ...
        // Insert cin to be the second-to-last bit, because the last bit is the sign bit
  ...
  int evala_DIV(Bundle& in0, Bundle& in1, Bundle& out) {
    ...
    for (u32 i = 0; i < len; i++) {
      if (i == 0) {
        ...
        if (dvg_status < 0) return dvg_status;
  ...
  int evala_DVG(Bundle& in0, Bundle& in1, Bundle& out, Wire*& ret) {
    ...
    if (sub_status < 0) return sub_status;

    int inv_status = evalw_INV(sub_res.back(), ret);
    if (inv_status < 0) return inv_status;
  ...
  int evalb_AND(Bundle& in0, Bundle& in1, Bundle& out) {
    ...
    for (u32 i = 0; i < len; ++i) {
      ...
      if (and_status < 0) return and_status;
      out.add(w);
    }
    return 0;
  }
...
int evalb_INV(Bundle& in, Bundle& out) {u32 len = in.size(); Wire* w;
  ...
  int evalc_LA(Bundle& in0, Bundle& in1, Bundle& out) {
    ...
    for (u32 i = 0; i < len; ++i) {
      ...
      if (xor_status < 0) return xor_status;
    ...
    for (u32 i = 0; i < len; ++i) {
      ...
      if (or_status < 0) return or_status;
      ret = tmp;
    }
  ...
  int evalc_LEE(Bundle& in0, Bundle& in1, Bundle& out) {
    ...
    if (le_status < 0) return le_status;
    int eq_status = evalc_EQ(in0, in1, ret_eq);
    if (eq_status < 0) return eq_status;
      ...
      if (mgc.is_input_wire(in)) {
        ...
        if (mgc.has_invert_wire(in)) {
          ...
          if (in->m_v >= 0) {
            ...
            write_gate(parent_gate->m_op, parent_gate->m_in0, parent_gate->m_in1, ret);
          }
        }
      }
    }
