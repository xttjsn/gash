...
Ast *new_ifelse(Ast *cond, Ast *if_ast, Ast *else_ast, Scope *if_scope,
                Scope *else_scope, Scope *prev_scope) {
  Ifel *aifel = new Ifel;
...
Ast *new_for(Ast *init, Ast *cond, Ast *inc, Ast *todo, Scope *for_scope,
             Scope *prev_scope) {
  For *afor = new For;
...
void dir_port(u32 port) { mectx.m_port = port; }

void dir_role(RoleType role) { mectx.m_role = role; }
...
void evalast_num(Num *num, Bundle &bret) { num2bundle(num->m_val, bret); }
...
void evalast_vdf(Vardef *vdf, Bundle &bret) {
  ...
  if (vdf->m_sym->m_type == NUM) {
    ...
    bret = sym_n->m_bundle;
  } else if (vdf->m_sym->m_type ==
             ARRAY) { // Array symbol: return all its bundle as a single one
...
i64 evalast_n(Ast *ast) {
  ...
  default: { FATAL("Unsupported evalast_n for type : " << ast->m_nodetype); }
  }
...
void evalast_n_num(Num *num, i64 &ret) { ret = num->m_val; }

void write_circuit() { mgc.write(); }

void write_data() { mgc.write_input(); }
